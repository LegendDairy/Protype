...
public:
	cpu_c(uint32_t flags, uint8_t id,  uint32_t f, uint32_t bootstrap);
...
cpu_c::cpu_c(uint8_t apic_id, uint32_t proc_id, uint32_t f, uint32_t bootstrap)
{
	id 		= apic_id;
	core_id 	= proc_id;
	numa_dom 	= 0;
	...
	flags		= f;
	next		= 0;
	scheduler	= new scheduler_c scheduler(apic_id, bootstrap );
}
